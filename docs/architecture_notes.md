...
값을 꺼내는 도구는 **역할에 따라 나눈다.** `<title>` · `<meta>` · `<link rel="canonical">` ·
JSON-LD `<script>` 처럼 **HTML 구조를 읽는 것은 BeautifulSoup**이 맡고, 이미 텍스트가 된
문자열에서 사이트별 접미사를 벗기는 **정제 규칙은 정규식**이 맡는다.

HTML 속성은 순서가 무의미한데 정규식은 왼쪽에서 오른쪽으로 읽으므로 순서를 강제할 수밖에 없다.
이 미스매치가 실제로 버그 2건을 만들고 있었다 — `<meta content="..." name="description">` 과
`<link href="..." rel="canonical">` 은 둘 다 유효한 HTML인데 값을 읽지 못했다. 예외도 로그도
없이 필드가 비는 종류라, Saramin의 경우 canonical 재요청이 조용히 스킵될 수 있었다.

백엔드는 표준 라이브러리 `html.parser` 를 쓴다. 워커는 네트워크 대기가 지배적이라 lxml의 파싱
속도 이점이 처리량에 드러나지 않고, C 확장이라 이미지만 커진다.

반대로 `strip_html_tags()` 는 정규식으로 남겨두었다. 본문의 줄바꿈 규칙이 바뀌면
`description_text` 가 달라지고, 그러면 **`content_hash` 가 기존 적재분과 어긋나** 중복 판정이
깨진다. 도구로서 더 나은 것과 지금 바꿔도 되는 것은 다른 문제다.
...
정규식 → HTML 파서 교체에서는 성격이 다른 구멍이 나왔다. 뮤테이션 12종 중 처음엔 7종만
잡혔는데, 살아남은 것 중에 **리팩터링 자체가 만들어낸 코드 경로**가 있었다. 정규식은 `rel` 과
`href` 를 한 번에 요구해서 "태그는 있는데 속성이 없는" 상태가 존재하지 않았다. 파서로 바꾸며
조회가 2단계로 나뉘자 그 상태가 새로 생겼고(가드가 없으면 `KeyError`), 기존 테스트가 그것을
커버하지 못하는 것은 당연했다. **구조를 바꾸면 테스트도 새 구조에 맞춰 늘려야 한다.**
구멍 5건을 메워 12/12 검출.
